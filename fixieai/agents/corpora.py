...
class CorpusRequest(dataclasses_json.DataClassJsonMixin):
    ...
    Request flow:
        For each request, agents should return:
            a set of paritions for the corpus, optionally with
    ...
    continuation_token: Optional[bytes] = dataclasses.field(
        default=None,
        metadata=dataclasses_json.config(
            encoder=lambda token: token.decode() if token else None,
            decoder=lambda str_token: str_token.encode() if str_token else None,
        ),
    )
...
class CorpusPartition:
    ...
    continuation_token: Optional[bytes] = dataclasses.field(
        default=None,
        metadata=dataclasses_json.config(
            encoder=lambda token: token.decode("UTF-8") if token else None,
            decoder=lambda str_token: str_token.encode("UTF-8") if str_token else None,
        ),
    )
...
class CorpusPage(dataclasses_json.DataClassJsonMixin):
    ...
    documents: List[CorpusDocument] = dataclasses.field(
        metadata=dataclasses_json.config(
            encoder=lambda docs: [
                {
            ...
            decoder=lambda raw_docs: [
                CorpusDocument(
                    source_name=doc["source_name"],
                    content=doc["content"].encode(doc["encoding"]),
                    encoding=doc["encoding"],
                    mime_type=doc["mime_type"],
                )
                for doc in raw_docs
